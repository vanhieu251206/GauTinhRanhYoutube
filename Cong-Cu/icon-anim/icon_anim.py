...
Đều có pop-in (bật to + nảy nhẹ) ở 0.8s đầu. --nopop để bỏ.
...
def render(src, out, a):
    ...
    if bbox:
        item = item.crop(bbox)
    item = item.resize((max(1, round(item.width * a.height_px / item.height)), a.height_px), Image.LANCZOS)
    preset = a.preset
    if preset == "random":
    ...
    for f in range(a.frames or round(FPS * a.dur)):
        ...
        if a.nopop:
            ...
            pop, fade, settle = back(t / 0.8), min(1, t / 0.25), min(1, max(0, (t - 0.6) / 0.6))
        ...
        frame.alpha_composite(im, (int(W / 2 + drift - im.width / 2), int(H / 2 + bob - im.height / 2)))
...
def main():
    ...
    ap.add_argument("--preset", default="float", choices=list(PRESETS) + ["random"])
    ...
    for i, src in enumerate(files, 1):
        ...
        out = os.path.join(od, a.name if (a.name and len(files) == 1) else base + "_anim.mov")
